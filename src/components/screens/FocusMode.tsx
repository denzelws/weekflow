  ...
  return (
    ...
    >
      <div className="flex flex-col flex-1 pt-8">
        ...
        {/* Progress strip */}
        <div className="mb-8 anim-hidden animate-fade-in" style={{ animationFillMode:'forwards' }}>
          <div className="flex justify-between mb-2">
            <span className="text-label">Tarefa {activeFocusIdx + 1} de {totalSelected}</span>
            <span className="text-label text-primary">{Math.round(progress * 100)}%</span>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-8 relative">
          ...
          {/* CURRENT FOCUS label */}
          <span className="text-label text-primary tracking-widest">CURRENT FOCUS</span>
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-label">
              {timerActive ? 'Tocando — clique para pausar' : 'Clique para iniciar timer'}
            </span>
          </div>
            ...
            {showOthers && (
              <div className="mt-2 flex flex-col gap-2 animate-fade-in">
                {remaining.map(task => task && (
                  ...
                  >
                    <p className="text-body-sm text-on-muted">{task.title}</p>
                  </div>
                ))}
              </div>
            )}
