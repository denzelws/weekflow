...
export function WeekKickoff() {
  ...
  return (
    <AppShell header={<Header profile={profile} />} className="px-6">
      <div
        className="pt-8 pb-6 anim-hidden animate-slide-up delay-0"
      ...
      <div
        className="mb-6 anim-hidden animate-slide-up delay-100"
        style={{ animationFillMode: "forwards" }}
      >
        <ProgressBar value={0} label="Progresso da semana" className="mb-1" />
      </div>

      <div
        className="flex gap-2 mb-8 anim-hidden animate-slide-up delay-150"
        style={{ animationFillMode: "forwards" }}
      >
        {days.map((day) => {
          ...
          return (
            ...
            >
              <span
                className={`text-label ${isToday ? "text-primary" : "text-on-muted"}`}
              >
                {label}
              </span>
              {isToday && <span className="w-1 h-1 rounded-full bg-primary" />}
            </div>
          );
        })}
      </div>
      ...
      <div
        ...
        style={{
          animationDelay: `${200 + tasks.length * 50 + 100}ms`,
          animationFillMode: "forwards",
        }}
      >
        <Button
